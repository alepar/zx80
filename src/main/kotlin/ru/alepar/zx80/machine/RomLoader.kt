...
/**
 * Loads the Sinclair 48K ROM image from the classpath. The ROM is downloaded at build time by the
 * Gradle `downloadRom` task and bundled as a resource at `/rom/48.rom`.
 */
object RomLoader {
    ...
    fun load48k(): ByteArray {
        val stream =
            RomLoader::class.java.getResourceAsStream(RESOURCE_PATH)
                ?: error(
