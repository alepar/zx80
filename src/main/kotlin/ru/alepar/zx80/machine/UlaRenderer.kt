...
/**
 * Reads ZX Spectrum 48K screen RAM (0x4000-0x5AFF) and produces a 256x192 BufferedImage. Headless;
 * no host window. M2.4 wraps the output in a display; M2.8 will add a border.
 *
 * `flashOn` selects the flash phase: false = ink and paper as written; true = ink and paper swapped
 * for cells with attribute bit 7 set. The caller (M2.4) toggles this every 16 frames.
 *
 * Pixel-data layout (0x4000-0x57FF, 6144 bytes): rows are interleaved into three banks of 64 rows
 * each. Address for screen row y:
 *
 *     0x4000 | ((y and 0xC0) shl 5) | ((y and 0x07) shl 8) | ((y and 0x38) shl 2)
 *
 * Attributes (0x5800-0x5AFF, 768 bytes): one byte per 8x8 cell, laid out linearly. Byte format: bit
 * 7 : FLASH bit 6 : BRIGHT bits 3-5 : PAPER color (0..7) bits 0-2 : INK color (0..7)
 *
