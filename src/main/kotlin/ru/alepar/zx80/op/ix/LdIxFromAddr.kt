...
/**
 * `LD IX/IY, (nn)` — load idx from memory at the 16-bit address nn. 20 T-states. R+=2. PC+=4. No
 * flag changes.
 */
class LdIxFromAddr(private val idx: IndexReg) : Op {
    override val operandLength = 2
    override val baseCycles = 20

    override fun execute(cpu: Cpu, mem: Memory) {
        val addr = mem.readWord(cpu.pc + 2)
        idx.write(cpu, mem.readWord(addr))
